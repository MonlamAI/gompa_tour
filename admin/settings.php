...
if(isset($_POST) & !empty($_POST)){
    ...
    if(empty($_POST['copyright'])){$errors[] = "Copyright is Required";}
    ...
    if(empty($errors)){
        ...
        $urlsql = "UPDATE settings SET value=?, updated=NOW() WHERE name='copyright'";
        $urlresult = $db->prepare($urlsql);
        $urlresult->execute(array($_POST['copyright']));
    }
}
// Create CSRF token
...
<div id="page-wrapper" style="min-height: 345px;">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">སྒྲིག་འགོད།</h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    དྲ་རྒྱའི་མ་ལག་ཀྱི་སྒྲིག་འགོད་ཁག...
                </div>
                        ...
                        <div class="col-lg-6">
                            <form role="form" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    ?>
                                    <label>དྲ་བའི་གློག་འཕྲིན་ཁ་བྱང་།</label>
                                ...
                                <div class="form-group">
                                    ...
                                    ?>
                                    <label>ཐོ་ཞུགས་དེབ་སྐྱེལ།</label>
                                    <label class="radio-inline">
                                        <input type="radio" name="userreg" id="optionsRadiosInline1" value="yes" <?php if($userreg['value'] == 'yes'){ echo "checked"; } ?>>ཡིན། 
                                    </label>
                                    <label class="radio-inline">
                                        <input type="radio" name="userreg" id="optionsRadiosInline2" value="no" <?php if($userreg['value'] == 'no'){ echo "checked"; } ?>>མིན།
                                    </label>
                                </div>
                                <div class="form-group">
                                    <?php
                                    ...
                                    ?>
                                    <label>ཤོག་ངོས་རེ་ལ་བརྗོད་གཞིའི་གྲངས།</label>
                                ...
                                <div class="form-group">
                                    ...
                                    ?>
                                    <label>སྦེལ་ཐག་ཁག་གཙང་བཟོ།</label>
                                    <label class="radio-inline">
                                        <input type="radio" name="cleanurls" id="optionsRadiosInline1" value="yes" <?php if($url['value'] == 'yes'){ echo "checked"; } ?>>ནུས་ལྡན། 
                                    </label>
                                    <label class="radio-inline">
                                        <input type="radio" name="cleanurls" id="optionsRadiosInline2" value="no" <?php if($url['value'] == 'no'){ echo "checked"; } ?>>ནུས་མེད།
                                    </label>
                                </div>
                                
                                <div class="form-group">
                                    <?php
                                        $copyrightsql = "SELECT * FROM settings WHERE name='copyright'";
                                        $copyrightresult = $db->prepare($copyrightsql);
                                        $copyrightresult->execute();
                                        $copyright = $copyrightresult->fetch(PDO::FETCH_ASSOC);
                                    ?>
                                    <label>དྲ་བའི་བདག་དབང་།</label>
                                    <input class="form-control" name="copyright" placeholder="Enter Site Title" value="<?php echo $copyright['value']; ?>">
                                </div>

                                <button type="submit" class="btn btn-primary">ནང་འཇུག</button>
                                <button type="reset" class="btn btn-danger">སླར་གསོ།</button>
                            </form>
                        </div>
                        <!-- /.col-lg-6 (nested) -->   
                    <!-- /.row (nested) -->
